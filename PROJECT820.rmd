...
Box.test(ts_territories.2[,4], type="Ljung-Box")
Box.test(ts_territories.2[,5], type="Ljung-Box")
Box.test(ts_territories.2[,6], type="Ljung-Box")
#generally reject the null that these are stationary time series

#stationarity might indicate that there is an equally likely 
...
options(warn = -1)
#non-sationary has a large p-value
adf.test(ts_ON.2[,4])
adf.test(ts_ON.2[,5])
adf.test(ts_ON.2[,6])
...
kpss.test(ts_territories.2[,5], null = "Trend")##reject the null that this is trend stationary
kpss.test(ts_territories.2[,6], null = "Trend")
##cannot reject the null of stationarity, p-value indicates they could be stationary 

```
```{r preliminary time series analysis-3}
##try ARIMA model

##time series regression [tsr]-> tslm
tsr_ON<- tslm(formula= ts_ON.2[,4]~ts_ON.2[,2]+ts_ON.2[,3], data=ts_ON.2)
summary(tsr_ON)
tsr_ON.5<- tslm(formula= ts_ON.2[,5]~ts_ON.2[,2]+ts_ON.2[,3], data=ts_ON.2)
summary(tsr_ON.5)
tsr_ON.6<- tslm(formula= ts_ON.2[,6]~ts_ON.2[,2]+ts_ON.2[,3], data=ts_ON.2)
summary(tsr_ON.6)
tsr_ON.7<- tslm(formula= ts_ON.2[,7]~ts_ON.2[,2], data=ts_ON.2)
summary(tsr_ON.7)
tsr_ON.8<- tslm(formula= ts_ON.2[,8]~ts_ON.2[,2], data=ts_ON.2)
summary(tsr_ON.8)

tsr_QC<- tslm(formula = ts_QC.2[,4]~ts_QC.2[,2]+ts_QC.2[,3], data = ts_QC.2)
summary(tsr_QC)
tsr_QC.5<- tslm(formula= ts_QC.2[,5]~ts_QC.2[,2]+ts_QC.2[,3], data=ts_QC.2)
summary(tsr_QC.5)
tsr_QC.6<- tslm(formula= ts_QC.2[,6]~ts_QC.2[,2]+ts_QC.2[,3], data=ts_QC.2)
summary(tsr_QC.6)
tsr_QC.7<- tslm(formula= ts_QC.2[,7]~ts_QC.2[,2], data=ts_QC.2)
summary(tsr_QC.7)
tsr_QC.8<- tslm(formula= ts_QC.2[,8]~ts_QC.2[,2], data=ts_QC.2)
summary(tsr_QC.8)
